...
export class MwPojmovaMapa {
  ...
  private vytvoritDataMapy(): void {
    const korenovyNadpis =
      document.getElementById("firstHeading")?.innerText || "Koreň";
  ...
  private spracovatNadpis(nadpis: HTMLHeadingElement): void {
    ...
    const nazov =
      nadpis.querySelector(".mw-headline")?.textContent ||
      nadpis.textContent ||
      "";
  ...
  private vykreslitMapu(): void {
    ...
    this.pojmova_mapa = new Network(
      this.elementMapy,
      dataSiete,
      this.vytvorNastavenia(),
    );
...
export default function vykreslitMapu() {
  const obsahStranky = document.querySelector(
