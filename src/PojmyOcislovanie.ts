...
function zformatovatCislo(
  nadradeneCislo: string | undefined,
  aktualnyPojem: number,
): string {
  return nadradeneCislo
    ? `${nadradeneCislo}.${aktualnyPojem}`
    : `${aktualnyPojem}`;
}

function zformatovatId(
  nadradeneCislo: string | undefined,
  aktualnyPojem: number,
): string {
...
export default function ocislujPojmy(): void {
  const nadradeneCislo = document.getElementById("sidebar-pojmy-config")
    ?.dataset.cislo;
